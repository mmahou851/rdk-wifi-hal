...
static void nl80211_connect_event(wifi_interface_info_t *interface, struct nlattr **tb)
{
    ...
    if ((tb[NL80211_ATTR_STATUS_CODE] == NULL) || nla_len(tb[NL80211_ATTR_STATUS_CODE]) != sizeof(status)) {
        wifi_hal_error_print("%s:%d: status code attribute absent or invalid status code attribute length:%u\n",
    ...
    if ((tb[NL80211_ATTR_MAC] == NULL) || nla_len(tb[NL80211_ATTR_MAC]) != sizeof(mac)) {
        wifi_hal_error_print("%s:%d: mac attribute absent or invalid mac attribute length:%u\n",
